                ...
                foreach (var ally in bestAllies)
                {
                    if (aaa == ally)
                    {
                        if (ObjectManager.Player.CharacterName == "Janna" ||
                            ...
                            ObjectManager.Player.CharacterName == "Karma")
                        {
                            if (SupportAIO.SpellBlocking.EvadeTargetManager.Menu["whitelist"][
                                        ally.CharacterName.ToLower()] &&
                                aaa.Distance(ObjectManager.Player) < SupportAIO.Common.Champion.E.Range)
                            {
                                SupportAIO.Common.Champion.E.CastOnUnit(aaa);
                            }
                        }
                        if (ObjectManager.Player.CharacterName == "Lux" || ObjectManager.Player.CharacterName == "Sona" ||
                            ObjectManager.Player.CharacterName == "Taric")

                        {
                            if (SupportAIO.SpellBlocking.EvadeTargetManager.Menu["whitelist"][
                                        ally.CharacterName.ToLower()] &&
                                aaa.Distance(ObjectManager.Player) < SupportAIO.Common.Champion.W.Range)
                            {
                                SupportAIO.Common.Champion.W.CastOnUnit(aaa);
                            }
                        }
                    }
